      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '12px' }}>
        {block.items.map((item, i) => (
          <div key={i} className="bg-gray-800 rounded-xl p-3 text-center">
            ...
            <p className="text-xs mt-1" style={{ color: 'var(--gp-muted, #94a3b8)' }}>{item.label}</p>
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${block.columns ?? 3}, 1fr)`, gap: '12px', marginBottom: '12px' }}>
        {block.items.map((item, i) => (
          <div key={i} className="bg-gray-800 rounded-xl p-2 text-center relative group/stat overflow-hidden">
            <button onClick={() => remove(i)} className="absolute top-1 right-1 text-gray-600 hover:text-red-400 opacity-0 group-hover/stat:opacity-100 z-10"><Trash2 size={11} /></button>
            {/* Use a single row of inputs that scales to available width */}
            <div className="flex items-baseline justify-center min-w-0">
              <input value={item.prefix ?? ''} onChange={e => update(i, { prefix: e.target.value })} placeholder="$"
                className="bg-transparent text-blue-400 text-xs font-bold focus:outline-none text-center min-w-0"
                style={{ width: item.prefix ? `${Math.max(12, item.prefix.length * 8)}px` : '10px' }} />
              <input value={item.value} onChange={e => update(i, { value: e.target.value })}
                className="bg-transparent text-xl font-bold text-white focus:outline-none text-center min-w-0"
                style={{ width: `${Math.max(24, item.value.length * 14)}px` }} />
              <input value={item.suffix ?? ''} onChange={e => update(i, { suffix: e.target.value })} placeholder="+"
                className="bg-transparent text-blue-400 text-xs font-bold focus:outline-none text-center min-w-0"
                style={{ width: item.suffix ? `${Math.max(12, item.suffix.length * 8)}px` : '10px' }} />
            </div>
            <input value={item.label} onChange={e => update(i, { label: e.target.value })}
              className="bg-transparent text-xs text-gray-400 text-center w-full focus:outline-none mt-1 block truncate" /></div>
        ))}
      </div>
